...
void writeint(int value, int count);
...
void loop()
{
   ...
   switch( button )
   {
      ...
      case BUTTON_LEFT://this button allows it to save the nummber it's currently on
      {
        /*if (position >= 0)
        {
          position = 255;
        ...
        lcd.print( position,DEC );
         ...
         lcd.setCursor( 0,1);
         lcd.print( position,BIN );
          lcd.setCursor( 10,1);
          lcd.print( position,3);
         delay(200);*/
         byte address = 0;
        writeInt(address, count);
         break;
      }
      case BUTTON_UP:
      {
      ...
      case BUTTON_RIGHT: // this button outputs what the left button saved
      {
        byte address = 0;
...
void writeInt(int address, int val) // this function is for the left button
{
  byte *p = (byte *) &val;
...
int readInt(int address) //THis function is for the right
{
  int val;
